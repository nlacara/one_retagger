...
This particular version of the script is for retagging examples that come after
possessives ('s, her, their, our, etc). It processes data output by the script
get_poss_tokens.py. Run that first.
"""
...
output_file = './poss_data_retagged.json'
input_file = './poss_data.json'
...
for sent_list in tagged_sents[retagged_count:tagged_count]:
    ...
    for token in range(1, len(sent)): #Start from index 1 b/c we only want tokens after possessives
        if sent[token][0].lower() in ['one', 'ones']:# and \
            ...
            print("Preceding tag:", Fore.LIGHTGREEN_EX +  sent[token - 1][1] 
                  + Style.RESET_ALL + '\t' + "Current tag:", Fore.LIGHTCYAN_EX 
                  + sent[token][1] + Style.RESET_ALL)
    ...
    # If we reach the end of tagged sentences,
    # write the new tags
    retagged_sent = sent
